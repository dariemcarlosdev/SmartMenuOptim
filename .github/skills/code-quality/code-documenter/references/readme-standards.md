...
Template and guidelines for writing module/component READMEs in the NexTruzt.io codebase.
...
| Type | Layer | Purpose |
|------|-------|---------|
| `IEscrowService` | Application | Escrow lifecycle operations contract |
| `EscrowService` | Infrastructure | Implementation of escrow operations |
| `CreateEscrowCommand` | Application | CQRS command for escrow creation |
| `EscrowValidator` | Application | FluentValidation rules for escrow commands |
...
```csharp
// Via MediatR
var result = await mediator.Send(new CreateEscrowCommand
{
    BuyerId = buyerId,
    SellerId = sellerId,
...
### Querying Escrows
```csharp
var escrow = await mediator.Send(new GetEscrowQuery(escrowId), ct);
var list = await mediator.Send(new ListEscrowsQuery { Status = EscrowStatus.Funded }, ct);
```
...
|-----|------|---------|-------------|
| `MaxAmount` | decimal | 1000000 | Maximum escrow amount in base currency |
| `TimeoutDays` | int | 30 | Days before unfunded escrow expires |
...
**Consumed by:**
- `WebApi` — REST endpoints for escrow operations
...
| `ShowClosed` | `bool` | No | Include closed escrows (default: false) |
| `OnEscrowSelected` | `EventCallback<EscrowId>` | No | Fires when user clicks an escrow row |
